...
echo "<!DOCTYPE html>
	...
	<p align= 'center'	style ='font-size:20px; color:#000;'>projects list</p>
	<table>
		<thread>
		<tr>
			<th>Project Name</th>
...
$query = $conn->query("select distinct projects.pid,projects.pname, powner,description, requests.posttime, pstatus, sum(money) as sm, minfund,projects.starttime
	from fund join projects on fund.pid=projects.pid join requests on requests.pid=projects.pid join tags on projects.pid=tags.pid
	where (fstatus = 'pending' or fstatus='charged') and tag= '$tag' group by projects.pid,powner, requests.posttime, pstatus,minfund,projects.starttime 
